        ...
        <section className="glass-card overflow-hidden rounded-[2rem] p-5 sm:p-7 lg:p-8">
          <div className="grid gap-5 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
            <div className="relative z-10">
              ...
              <h1 className="mt-8 max-w-xl text-4xl font-bold tracking-[-0.045em] text-slate-950 sm:text-5xl">Cek gigi hari ini?</h1>
              <p className="mt-3 max-w-sm text-sm leading-6 text-slate-600">Ambil foto. Lihat area yang ditandai.</p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <LinkButton href="/app/scan">Mulai scan</LinkButton>
                <LinkButton href="/app/history" variant="secondary">Riwayat</LinkButton>
              </div>
              <p className="mt-5 text-xs font-semibold text-slate-500">Bukan diagnosis dokter.</p>
            </div>
        ...
        <Card>
          ...
          {latest ? (
            <>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">{latest.total_images} foto.</p>
              <Link className="mt-4 inline-block text-sm font-bold text-clinical-700" href={`/app/history/${latest.id}`}>
                Lihat detail
              </Link>
            </>
          ) : (
            <p className="mt-3 text-sm leading-6 text-slate-600">Belum ada skrining tersimpan.</p>
          )}
        </Card>
      </aside>
    </div>
